...
// ✅ ORİJİNAL FORMAT - SADECE set1, set2...
app.get('/last-cookies', (req, res) => {
    ...
    // 🎯 SETLER DİREKT COOKIE ARRAY'LERİ - CHROME UYUMLU
    successfulSets.forEach(set => {
        result[`set${set.set_id}`] = set.chrome_cookies; // 🎯 CHROME UYUMLU COOKIE'LER
    });
...
app.get('/', (req, res) => {
    res.json({
        ...
        endpoints: {
            ...
            '/last-cookies': 'Son cookie\'leri göster (Orijinal format - Chrome uyumlu)',
...
├── Orijinal /last-cookies: ✅ AKTİF
├── Chrome /chrome-cookies: ✅ AKTİF
└── Tam Chrome Uyumluluk: ✅ AKTİF

🌐 ENDPOINT'LER:
├── /last-cookies - Orijinal format (Chrome uyumlu)
├── /chrome-cookies - Tek array formatı
...
app.get('/stats', (req, res) => {
    ...
    res.json({
        ...
        chrome_features: {
            ...
            endpoints: {
                last_cookies: 'orijinal_format',
                chrome_cookies: 'tek_array'
            }
        }
    });
});
...
(async () => {
  try {
    ...
    if (loaded && loaded.length > 0) {
      lastCookies = loaded;
      console.log(`✅ ${loaded.length} cookie seti RAM'e yüklendi`);
    } else {
...
app.listen(PORT, async () => {
    ...
    console.log('📍 /last-cookies - Son cookie\'leri göster (Orijinal format - Chrome uyumlu)');
    console.log('📍 /chrome-cookies - Chrome eklentisi için optimize');
